...
export function useHomeMotion() {
  ...
  useEffect(() => {
    ...
    const sync = () => {
      sections.forEach((el) => { el.dataset.running = String(visible.has(el) && !document.hidden && !media.matches); });
    ...
    reveals.forEach((el) => {
      if (!media.matches && el.getBoundingClientRect().top > window.innerHeight) el.classList.add("reveal-pending");
      observer.observe(el);
    });
